...
const toDisplayString$1 = (val) => {
  return isString$2(val) ? val : val == null ? "" : isArray$3(val) || isObject$5(val) && (val.toString === objectToString$2 || !isFunction$2(val.toString)) ? JSON.stringify(val, replacer, 2) : String(val);
};
...
function hasCallback(args) {
  if (isPlainObject$4(args) && [API_SUCCESS, API_FAIL, API_COMPLETE].find((cb) => isFunction$2(args[cb]))) {
    return true;
  }
  return false;
}
...
function populateParameters(fromRes, toRes) {
  ...
  const parameters = {
    appId: "wx1508859d78a3f436",
    appName: "designated-driver-minprogram-passenger",
    appVersion: "3.1.08",
    appVersionCode: "100",
...
function patchMPEvent(event) {
  if (event.type && event.target) {
    ...
    if (isPlainObject$4(event.detail) && hasOwn$2(event.detail, "checked") && !hasOwn$2(event.detail, "value")) {
...
function createCoreContext(options = {}) {
  ...
  const fallbackLocale = isArray$2(options.fallbackLocale) || isPlainObject$2(options.fallbackLocale) || isString$1(options.fallbackLocale) || options.fallbackLocale === false ? options.fallbackLocale : locale;
  const messages = isPlainObject$2(options.messages) ? options.messages : { [locale]: {} };
...
const Translation = {
  ...
  setup(props, context) {
    ...
    return () => {
      ...
      assign({}, attrs);
      return isString$1(props.tag) ? h(props.tag) : isObject$3(props.tag) ? h(props.tag) : h(Fragment);
    };
  }
};
...
function editScope(payload, i18n) {
  ...
  if (composer) {
    ...
    } else if (field === "fallbackLocale" && (isString$1(payload.state.value) || isArray$2(payload.state.value) || isObject$3(payload.state.value))) {
...
function buildURL(url, params, paramsSerializer) {
  ...
  if (paramsSerializer) {
    ...
    forEach(params, function serialize(val, key) {
      ...
      forEach(val, function parseValue2(v) {
        if (isDate(v)) {
          v = v.toISOString();
        } else if (isObject(v)) {
          v = JSON.stringify(v);
...
const adapter = (config) => {
  return new Promise((resolve2, reject) => {
    ...
    if (config.method === "UPLOAD") {
      ...
      requestTask = index.uploadFile(__spreadValues(__spreadValues(__spreadValues({}, _config), otherConfig), mergeKeys$1(optionalKeys, config)));
    } else if (config.method === "DOWNLOAD") {
      const optionalKeys = [
        "timeout",
        "filePath"
      ];
      requestTask = index.downloadFile(__spreadValues(__spreadValues({}, _config), mergeKeys$1(optionalKeys, config)));
    } else {
      const optionalKeys = [
        "data",
        "method",
      ...
      requestTask = index.request(__spreadValues(__spreadValues({}, _config), mergeKeys$1(optionalKeys, config)));
    }
    if (config.getTask) {
...
const mergeConfig = (globalsConfig, config2 = {}) => {
  ...
  if (method === "DOWNLOAD") {
    ...
    config = __spreadValues(__spreadValues({}, config), mergeKeys(downloadKeys, globalsConfig, config2));
